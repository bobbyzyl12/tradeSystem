...
$(function(){
			...
			$.ajax({                           	  
                ...
                {  
                	...
                	if(productList!=null)
                	{
                		...
                		objSelect.options.add(new Option("请选择交易货物",0));
                		for (var i=0;i<productjson.length;i++)
                		{
                			objSelect.options.add(new Option(productjson[i].productName,productjson[i].tProductId));
                		}
                	}
                 }
             });          
	...
	$("#product").on("change",function(){
		...
		var brokercompanyid=$("#brokercompany").val();
		var bproductid=$("#product").val();
		alert("brokercompanyid:	"+brokercompanyid);
		alert("bproduct:		"+bproductid);
		var div = document.getElementById("myChart2");
		
		$.ajax({                           	  
            url: '${ctx}/MySystem/MarketDepth/findMarketDetail',                  
            type: 'POST',                  
            data: {'bproductid':bproductid,
            	'brokercompanyid':brokercompanyid},
            dataType:"json",
            success: function (detailList) 
            {  
            	var str = JSON.stringify(detailList);
            	alert(str);
            	if(str=="[]")
            	{
            	...
            	else
            	{
            		...
            		/*for (var i=0;i<productjson.length;i++)
            		{
            			alert("price:"+productjson[i].price);
            			alert("quantity:"+productjson[i].quantity);
            			alert("ifBuy:"+productjson[i].ifBuy);*/
            			
            			
            			
            			var ChartLabels = new Array();
        				for(var i in productjson) {
        					ChartLabels.push(productjson[i].price);
        				}
        				...
        				for(var i in productjson) {
        					if(productjson[i].ifBuy==1){
        						for(var j in ChartLabels){
        							if(productjson[i].price == ChartLabels[j]){
        								data1[j] = data1[j]+productjson[i].quantity;
        							}
        						}
        					}
        					else if(productjson[i].ifBuy==0){
        						for(var j in ChartLabels){
        							if(productjson[i].price == ChartLabels[j]){
        								data2[j] = data2[j]+productjson[i].quantity;	
        							}
        						}
        					}
        				}
                 ...
                 <div class="col-md-12">
                 	...
                 	<div class="panel" data-collapsed="0">
                        ...
                        <div class="panel-body">
                            ...
                            <div id="myChart2" style="width:400px;height:400px" style="visibility: hidden;">
